...
| [05-phase-ideation.md](./05-phase-ideation.md) | Ideation agent — **interactive** multi-turn conversation, per-turn container, finalization |
| [06-phase-generation.md](./06-phase-generation.md) | Generation agent — **autonomous** code, GitHub, Vercel, constraints |
| [07-phase-maintain.md](./07-phase-maintain.md) | Maintain agent — **autonomous** SEO, AEO, incidents, cron, confidence |
| [08-context-engine.md](./08-context-engine.md) | Context Engine — wiki-based context system powering all agents |

## Key Decisions (locked)

- **Flow shape:** Ideation is **interactive** (back-and-forth chat between user and agent). Generation and Maintain are **autonomous** one-shot phases. User input after ideation is limited to blockers and approvals.
...
- **Context store:** S3 — markdown files per project, agent pulls selectively guided by `index.md`
- **Metadata store:** RDS Postgres — project status, phase state, ideation messages, secrets (encrypted), approvals
- **Container lifecycle:**
  - Ideation: **per-turn** — one container per user message, exits after one reply
  - Generation / Maintain: **one-shot autonomous** — spin up, run to completion, tear down
